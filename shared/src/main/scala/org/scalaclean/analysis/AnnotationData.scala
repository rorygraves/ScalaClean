 ...
 */
case class AnnotationData(posOffsetStart: Int, posOffsetEnd: Int, fqName: String, values: Map[String, String])
    extends StandardExtensionData {

  import scala.collection.SortedMap

  override def restToCSV: String = {
    val csv =
      if (values.isEmpty) "" else values.toList.sortBy(_._1).map(e => s"${e._1},${e._2}").mkString(",", ",", "")
    s""",$fqName$csv"""
  }
