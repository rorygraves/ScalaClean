...
class ElementsWriter(file: File) {
  ...
  def commonPrefix(model: ModelSymbol): String =
    s"${model.ioToken},${model.legacyCsvIDString},${model.newCsvString},${model.tree.symbol.flags.toHexString},${model.sourceFile},${model.posStart},${model.posEnd},${model.posFocus},${model.traversal}"
